...
```{r}
head(genes_variantes)
```


```{r}
suma_variantes = colSums(genes_variantes[-1])
genes_filtrados = suma_variantes > 40
genes_variantes_min40 = genes_variantes[,genes_filtrados]
```

```{r}
genes_variantes_min40$grupo = genes_variantes$grupo
genes_variantes_min40$grupo = as.factor(genes_variantes_min40$grupo)
genes_variantes_min40 = genes_variantes_min40 %>%
    select(last_col(), everything())
```


```{r}
pca_result = prcomp(genes_variantes_min40[,-1],center = TRUE )
```
...
selected_pcs_df$grupo <- as.factor(genes_variantes_min40$grupo)
...
saveRDS(pca_training.svm.lineal, "genset_var85_min40_svml")
```
```{r}
...
saveRDS(pca_training.svm.radial, "genset_var85_min40_svmr_auto")
```
```{r}
...
saveRDS(pca_training.svm.radial_manual, "genset_var85_min40_svmr_man")
```
...
saveRDS(pca_training.svm.polinomial, "genset_var85_min40_svmp")
```
...
tunegrid.rf.1 = expand.grid(mtry = c(raiz_variables-35,raiz_variables-25,raiz_variables-15,raiz_variables-5 ,raiz_variables,raiz_variables+10, raiz_variables+30, raiz_variables+60, raiz_variables+100),
...
saveRDS(pca_training.rf.1, "genset_var85_min40_rf")
```
...
saveRDS(pca_diab.models.1, "genset_var85_min40_nb")
```
...
pca_matriz.confusion.rf$byClass[c("Sensitivity","Specificity","F1","Precision")]
```
```{r}
set.seed(1234)
...
pca_matriz.confusion.svml$byClass[c("Sensitivity","Specificity","F1","Precision")]
```
```{r}
set.seed(1234)
...
pca_matriz.confusion.svmr$byClass[c("Sensitivity","Specificity","F1","Precision")]
```
...
pca_matriz.confusion.svmp$byClass[c("Sensitivity","Specificity","F1","Precision")]
```
...
pca_matriz.confusion.nb$byClass[c("Sensitivity","Specificity","F1","Precision")]
``` 
...
pca_matriz.confusion.svmr_manual$byClass[c("Sensitivity","Specificity","F1","Precision")]
